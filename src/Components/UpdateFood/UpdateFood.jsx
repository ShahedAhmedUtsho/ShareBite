...
const UpdateFood = () => {
  const { user, openErrorModal, setModelMessage, setModelHead, openSuccessModal } = useContext(AuthContext);
  const params = useParams();
  const [date, setDate] = useState(null);
  const [sub, setSub] = useState("available");

  const { isLoading,  data: Food ,refetch } = useQuery({
    queryKey: [`${user.uid}/${params.id}`],
    queryFn: async () => {
      const response = await fetch(`https://test-tau-green-45.vercel.app/manage/${user.uid}/${params.id}`);
  ...
  useEffect(() => {
    document.title ="ShareBite | Update food";
  }, []);
  ...
  const handleCraft = event => {
    ...
    const expired_date = date ? format(date ?? new Date(), 'PPP') : Food.expired_date;
    ...
    const axiosURL = `https://test-tau-green-45.vercel.app/manage/${user.uid}/${params.id}` ;
    console.log(axiosURL)
   

    axios.put(axiosURL, UpdateFood)
      .then(response => {
        console.log("Successful update:", response.data);

setModelHead("Okey")
        setModelMessage("your food update successfully")
        openSuccessModal()
        refetch()
      })
      .catch(error => {
  ...
  return (
   <div className='w-full min-h-[90vh] bg-slate-800  bannarBG'>
    ...
    <h2 className='md:text-4xl text-slate-300
     text-center mt-[6%]'> Update your  <span style={{  fontWeight: 'bold' }}>
     ...
     <form  onSubmit={handleCraft} className="mx-auto mt-[2%] max-w-5xl space-y-2 my-20 rounded-lg border   p-8   shadow-md bg-[#00000034] ">
      ...
      <fieldset className="space-y-1">
        ...
        <div className="relative">
          <Input required id="food_name" name='food_name' defaultValue={Food?.food_name} placeholder="Enter food name" type="text" className="ps-11 rounded-sm text-slate-200 bg-slate-700  " />
          <Icon>
      ...
      <fieldset className="space-y-1">
        ...
        <div className="relative">
          <Input  required name='photoURL' placeholder="Enter photoURL" defaultValue={Food?.photoURL} className="ps-11 text-slate-200 bg-slate-700 rounded-sm" />
          <Icon>
      ...
      <fieldset className="space-y-1">
        ...
        <div className="relative bg-slate-700 ">
          <Input required name='food_quantity' defaultValue={Food.food_quantity} id="food_quantity" placeholder="Enter food_quantity" type="number" className="ps-11 rounded-sm bg-slate-700 text-slate-200 " />
          <Icon>
      ...
      <fieldset className="space-y-1">
        ...
        <div className="relative">
          <Input required name='pickup_location' defaultValue={Food.pickup_location} id="pickup_location" placeholder="Enter pickup_location" type="text" className="ps-11 rounded-sm bg-slate-700 text-slate-200 " />
          <Icon>
      ...
      <Popover.Action asChild  >
        <Button type='button'
          ...
          {date ? format(date ?? new Date(), 'PPP') : <span>{Food.expired_date}</span>}
        </Button>
      </Popover.Action>
...
<fieldset className="space-y-1.5   col-span-3">
      ...
      <Textarea defaultValue={Food.additional_notes} className='rounded-sm text-slate-200  bg-slate-700' required name='additional_notes' id="additional_notes" placeholder="Enter additional_notes" />
